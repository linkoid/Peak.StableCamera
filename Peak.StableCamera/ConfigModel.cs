...
internal readonly record struct ConfigModel(ConfigFile ConfigFile)
{
    ...
    public readonly ConfigEntry<bool>  HideFullBody        = ConfigFile.Bind("StableCamera", nameof(HideFullBody), false, "Hide the entire body. Will cause arms to be invisible in first person, but when TrackingPower is less than 1, this will prevent the body from blocking the view.");
